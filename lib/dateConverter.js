...
// Convert a date to time of day
function dateToTimeOfDayNumber(date) {
    const h = date.getHours() * 1000 * 60 * 60;
    const m = date.getMinutes() * 1000 * 60;
...
module.exports = {
    ...
    dateToNumber(date) {
        ...
        let dateNumber = Math.round((dateOnly - dateBase) / millisecondsInDay);
       
        // Get the daily fractal
        let timeNumber = dateToTimeOfDayNumber(date) / millisecondsInDay;

        // The date + time of day number in Excel format
        let number = dateNumber + timeNumber; 
